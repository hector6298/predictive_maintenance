...
with DAG(**dag_args) as dag:
    ...
    t2 = BashOperator(
        ...
        bash_command="""az ml workspace create --name {{ var.json.infrastructure_config.aml.workspace_name }} \
                                          --resource-group {{ var.json.infrastructure_config.resource_group }} \
                                          --location {{ var.json.infrastructure_config.azure_region }} \
                                          --container-registry "{{ ti.xcom_pull(task_ids='get_acr_id') }}"
        """
    )
    ...
    t4 = BashOperator(
        ...
        bash_command="""az iot hub create --name {{ var.json.infrastructure_config.iot.hub.name }} \
                                          --resource-group {{ var.json.infrastructure_config.resource_group }} \
                                          --sku {{ var.json.infrastructure_config.iot.hub.sku }} \
                                          --partition-count 2 \
                                          --location {{ var.json.infrastructure_config.azure_region }} 
        """
    )
    ...
    t5 = BashOperator(
        ...
        bash_command="""az storage account create \
                                        --name {{ var.json.infrastructure_config.iot.storage.name }} \
                                        --resource-group {{ var.json.infrastructure_config.resource_group }} \
                                        --location {{ var.json.infrastructure_config.azure_region }}  \
                                        --sku {{ var.json.infrastructure_config.iot.storage.sku }}  \
                                        --kind {{ var.json.infrastructure_config.iot.storage.kind }} \
                                        --access-tier {{ var.json.infrastructure_config.iot.storage.access_tier }}
        """
    )
    ...
    t7 = BashOperator(
        ...
        bash_command="""az storage container create --name {{ var.json.infrastructure_config.iot.storage.container_name }} \
                                                    --account-name {{ var.json.infrastructure_config.iot.storage.name }} \
    ...
    t8 = BashOperator(
        ...
        bash_command="""az iot hub routing-endpoint create \
                            ...
                            --hub-name {{ var.json.infrastructure_config.iot.hub.name }} \
                            --container {{ var.json.infrastructure_config.iot.storage.container_name }} \
                            --resource-group {{ var.json.infrastructure_config.resource_group }} \
                            --encoding avro
        """
    )
